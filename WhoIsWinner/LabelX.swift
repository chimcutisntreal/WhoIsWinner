...
class UILabelX: UILabel {
    ...
    func centreArcPerpendicular() {
        ...
        let l = str.utf8CString.count
        let attributes = [NSAttributedString.Key.font: font]
        
        let characters: [String] = str.utf8CString.map { String($0) } // An array of single character strings, each character in str
        var arcs: [CGFloat] = [] // This will be the arcs subtended by each character
        var totalArc: CGFloat = 0 // ... and the total arc subtended by the string
        
        // Calculate the arc subtended by each letter and their total
        for i in 0 ..< l {
            arcs += [chordToArc(characters[i].size(withAttributes: attributes as [NSAttributedString.Key : Any]).width, radius: radius)]
            totalArc += arcs[i]
        }
    ...
    func centre(text str: String, context: CGContext, radius r:CGFloat, angle theta: CGFloat, slantAngle: CGFloat) {
        ...
        let attributes = [NSAttributedString.Key.foregroundColor: r, NSAttributedString.Key.font: font] as [NSAttributedString.Key : Any]
        // Save the context
        context.saveGState()
